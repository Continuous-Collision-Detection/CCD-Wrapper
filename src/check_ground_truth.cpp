...
void read_CSV(const std::string inputFileName, std::vector<Args>& args)
{
    ...
    while (infile) // there is input overload classfile
    {
        ...
        if (s[0] != '#') {
            ...
            argi.method = CCDMethod::MULTIVARIATE_INTERVAL_ROOT_FINDER;
...
read_rational_CSV_get_string(const std::string inputFileName)
{
...
void run_single_query(int argc, char* argv[])
{
    ...
    Args args; // arguments are like: executable, filename, query id, is_vf,
               // result
    ...
    args.method = CCDMethod::MULTIVARIATE_INTERVAL_ROOT_FINDER;
    bool use_msccd = is_minimum_separation_method(args.method);

    // std::cout<<"method, "<<args.method<<" out of "<<
    // NUM_CCD_METHODS<<std::endl;
    igl::Timer timer;
    ...
    for (auto& entry : std::filesystem::directory_iterator(filename)) {
        ...
        //      std::cout<<"the ith don't match, i "<<i<<"rst, "<<result<<" ,
        //      "<<new_result<<std::endl;
        ...
        if (new_result != expected_result) {
            std::cout << "result don't match, groud, " << expected_result
                      << " ours, " << new_result << std::endl;

            if (new_result == 0) {
                std::cout << "false nega, " << filename << ", " << i
                          << std::endl;
                for (int j = 0; j < 8; j++) {
                    std::cout << "v" << j << " " << V(j, 0) << ", " << V(j, 1)
                              << ", " << V(j, 2) << std::endl;
                }
            }
        }
        ...
        if (result != expected_result) {
            if (result) {
                ...
                if (args.method
                    ...
                    || args.method == CCDMethod::MIN_SEPARATION_ROOT_PARITY) {
                    std::cerr
                        << fmt::format(
...
void write_csv(
    const Eigen::Matrix<std::string, Eigen::Dynamic, Eigen::Dynamic>& data,
    const std::string& filename)
{
    ...
    for (unsigned int i = 0; i < data.rows(); i++) {
        if (i % 100000 == 0) {
            fout.open(
                filename + "small_" + std::to_string(filenumber) + ".csv");
            // std::cout<<"file number, "<<filenumber<<std::endl;
            filenumber++;
        }
        fout << data(i, 0) << "," << data(i, 1) << "," << data(i, 2) << ","
...
void write_csv_data_result(
    const std::vector<std::array<std::string, 6>>& data,
...
int get_rational_vertices(int argc, char* argv[])
{
    ...
    if (debug_flag) {
        dirs.clear();
        dirs.push_back("/home/zachary/Development/ccd-queries/"
                       "erleben-cube-cliff-edges/vertex-face/");
    } else {
    ...
    for (int nm = 0; nm < dirs.size(); nm++) {
        ...
        for (auto& entry : std::filesystem::directory_iterator(filedir)) {
            ...
            Eigen::Matrix<std::string, Eigen::Dynamic, Eigen::Dynamic>
                All_Rational_V(all_V.rows(), all_V.cols() * 2);
            ...
            for (size_t i = 0; i < num_queries; i++) {
                ...
                for (int j = 0; j < 8; j++) {
                    ...
                    for (int k = 0; k < 3; k++) {
                        ...
                        // "<<r.get_denominator_str()<<" ";

                        double test_number = new_r.get_double(
                            r.get_numerator_str(), r.get_denominator_str());
                        // double(r.get_numerator())/double(r.get_denominator());
                        if (test_number != V(j, k)) {
                            std::cout << "\nnumber not match, number, "
                                      << V(j, k) << ", test number, "
                                      << test_number << ", numerator, "
                                      << r.get_numerator() << ", denominator, "
                                      << r.get_denominator() << std::endl;

                            if (r.get_denominator() == 0) {
                                std::cout << "the j and k are, " << j << " , "
                                          << k << std::endl;
                                std::cout << "numerator string, "
                                          << r.get_numerator_str() << std::endl;
                                std::cout << "denominator string, "
                                          << r.get_denominator_str()
                                          << std::endl;
                                std::cout << "dir, " << filedir << ", i, " << i
                                          << std::endl;
                            }
                        }
                        // if(r.get_denominator()!=0){
            ...
            Eigen::Matrix<std::string, Eigen::Dynamic, Eigen::Dynamic> tmpstr(
                2, 1);
            ...
            write_csv(
                All_Rational_V,
                filedir + "Rational_" + std::to_string(h5_nbr) + "_");
            filenumber++;
            std::cout << "finshed, " << entry.path().string() << std::endl;
        }
        std::cout << "there are " << rows << " rows in " << dirs[nm]
                  << std::endl;
    }
    return 0;
}
    ...
    for (int fnbr = 0; fnbr < files.size(); fnbr++) {
        ...
        for (int k = 0; k < args.size(); k++) {
            ...
            if (is_edge_edge != args[k].is_edge_edge) {
                ...
                // std::cout<<"switch method, is_edge_edge,
                // "<<is_edge_edge<<std::endl;
            }

            // for (auto& entry :
            // std::filesystem::directory_iterator(args[k].data_dir)) {
            ...
            if (old_name != args[k].data_dir) {
                ...
                all_V = read_rational_CSV_get_string(
                    args[k].data_dir); // args[k] get updated first, then update
                                       // all_V
                if (all_V.size() % 8 != 0) {
                    std::cout << "ERROR, all_V should have 8*query_number rows"
                              << std::endl;
                }
                ...
                write_csv_data_result(
                    all_V, ccd_results, args[k].data_dir + "_final.csv");
                ccd_results.clear();
            }
        }
    }
    std::cout << "successfully finish writing files in inputFileName"
              << std::endl;
}
struct write_format {
    std::string file;
    ...
    const int max_itr = 1e6)
{
    ...
    for (int fnbr = 0; fnbr < files.size(); fnbr++) {
        ...
        for (int k = 0; k < args.size(); k++) {
            if (fix_for_debug) {
                if (args[k].data_dir
                    != "/home/zachary/Development/ccd-queries/"
                       "erleben-cube-cliff-edges/edge-edge/"
                       "Rational_small_9.csv") {
                    continue;
                }
            }
            ...
            if (old_name != args[k].data_dir) {
                ...
                all_V = read_rational_CSV(
                    args[k].data_dir + "_final.csv", results);
            ...
            if (new_result != expected_result) {
                ...
                if (new_result == 0) {
                    std::cout << "false nega, " << args[k].data_dir << ", " << i
                              << std::endl;
                    for (int j = 0; j < 8; j++) {
                        std::cout << "v" << j << " " << V(j, 0) << ", "
                                  << V(j, 1) << ", " << V(j, 2) << std::endl;
                        if (j == 3)
                            std::cout << std::endl;
                    }
                    new_false_negatives++;
                    ...
                    if (method == CCDMethod::TIGHT_INTERVALS) {
                        std::cout << "false nega number, "
                                  << new_false_negatives << std::endl;
                        std::cout << "is edge? " << args[k].is_edge_edge
                                  << std::endl
                                  << std::endl;
                        std::cout << "number over all, " << total_number
                                  << std::endl;
                        exit(0);
                    }
                }
    ...
    std::cout << "average time, " << new_timing / double(total_number + 1)
              << std::endl
              << std::endl;
    std::cout << "percentage of early return, "
              << double(nbr_diff_tol) / total_number << std::endl;
    std::cout << "percentage of early and larger tol return, "
              << double(nbr_larger_tol) / total_number << std::endl;
    ...
    write_summary(
        ...
        new_false_positives, new_false_negatives,
        new_timing / double(total_number + 1));
    write_results_csv(
        folder + "method" + std::to_string(method) + "_is_edge_edge_"
            + std::to_string(is_edge_edge) + "_"
...
{ // method
    ...
    if (!run_only_simu) {
        ...
        run_rational_data_single_method(
            ms_flag, ms, tolerance, (CCDMethod)method, files, folder, tail,
            max_itr);
        files.clear();
...
void run_select(const int select)
{
    ...
    if (select == 0 || select == 1 || select == 2 || select == 3 || select == 4
        || select == 8 || select == 10 || select == 11) {
        ms_flag = false;

    } else {
        if (select == 5 || select == 6 || select == 7 || select == 9)
            ms_flag = true;
        else {
    ...
    select_one_method_to_run(
        folder, method, ms_flag, ms, tolerance, tail, false, max_itr);
}
...
void run_ms_methods_diff_eps(int start)
{
    ...
    for (int i = 0; i < ms.size(); i++) {
        select_one_method_to_run(
            folder, 9, ms_flag, std::stod(ms[i]), tolerance, ms[i], false,
            max_itr); // only simulation
...
void run_our_methods_diff_delta(int start)
{
    ...
    for (int i = 0; i < tolerance.size();
         i++) { // temporarly changed to run the last data
        select_one_method_to_run(
            folder, 9, ms_flag, ms, std::stod(tolerance[i]), tolerance[i], true,
            max_itr);
    }
}
...
int main(int argc, char* argv[])
{
    ...
    if (select_content == "1") {
        std::cout << "running different eps, input start number should be 0, "
                     "10, 20, 30,..."
                  << std::endl;
    ...
    // std::cout<<"args13653, "<<args[13653].data_dir<<", "<<args[13653].nbr<<",
    // is collision,"<<args[13653].result
...

    {
        ...
        //     bool is_ee=false;
        //     run_single_rational_csv_file(true,0, CCDMethod::TIGHT_INTERVALS,
        //     is_ee, file); run_single_rational_csv_file(true,0,
        //     CCDMethod::MIN_SEPARATION_ROOT_PARITY, is_ee, file);
        //     run_single_rational_csv_file(false,0,
        //     CCDMethod::RATIONAL_ROOT_PARITY, is_ee, file); std::cout<<"start
        //     BSC "<<std::endl; run_single_rational_csv_file(false,0,
        //     CCDMethod::BSC, is_ee, file); std::cout<<"start TIGHT_CCD
        //     "<<std::endl; run_single_rational_csv_file(false,0,
        //     CCDMethod::TIGHT_CCD, is_ee, file); std::cout<<"start FLOAT
        //     "<<std::endl; run_single_rational_csv_file(false,0,
        //     CCDMethod::FLOAT, is_ee, file);
    }
